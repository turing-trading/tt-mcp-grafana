...
func run(transport, addr, basePath, endpointPath string, logLevel slog.Level, dt disabledTools, gc mcpgrafana.GrafanaConfig, healthEnabled bool) error {
	...
	case "sse":
		if healthEnabled {
			return health.StartSSEWithHealth(s, addr, mcpgrafana.ComposedSSEContextFunc(gc), basePath)
		}
	...
	case "streamable-http":
		if healthEnabled {
			return health.StartStreamableHTTPWithHealth(s, addr, mcpgrafana.ComposedHTTPContextFunc(gc), endpointPath)
		}
...
func main() {
	...
	healthEnabled := flag.Bool("health-enabled", true, "Enable /healthz health check endpoint for server transports")
	...
	if err := run(transport, *addr, *basePath, *endpointPath, parseLevel(*logLevel), dt, grafanaConfig, *healthEnabled); err != nil {
		panic(err)
	}
}
